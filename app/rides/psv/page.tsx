...
export default function PSVRidesPage() {
  const router = useRouter();
  const [pickup, setPickup] = useState("");
  const [dropoff, setDropoff] = useState("");
  const [vehicleClass] = useState("PSV");
  const [selectedVehicle, setSelectedVehicle] = useState("");
  const [vehicles, setVehicles] = useState<Vehicle[]>([]);
  const [price, setPrice] = useState<number | null>(null);
  const [message, setMessage] = useState("");

  // Fetch available PSV vehicles
  useEffect(() => {
  ...
  const handleBooking = async () => {
    ...
    const basePrice = 150; // PSV pricing
    setPrice(basePrice);
    ...
    <>
      ...
      <main className="bg-black text-white min-h-[80vh] flex flex-col justify-center items-center p-6">
        <div className="w-full max-w-md">
          <h1 className="text-3xl font-extrabold mb-2 text-center">Book PSV Ride</h1>
          <p className="text-gray-400 text-center mb-6">Affordable public service vehicle rides for your daily commute.</p>
          ...
          >
            <option value="">Select a PSV Vehicle</option>
