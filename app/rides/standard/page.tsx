...
export default function StandardRidesPage() {
  const router = useRouter();
  const [pickup, setPickup] = useState("");
  const [dropoff, setDropoff] = useState("");
  const [vehicleClass] = useState("Standard");
  const [selectedVehicle, setSelectedVehicle] = useState("");
  const [vehicles, setVehicles] = useState<Vehicle[]>([]);
  const [price, setPrice] = useState<number | null>(null);
  const [message, setMessage] = useState("");

  // Fetch available Standard vehicles
  useEffect(() => {
  ...
  const handleBooking = async () => {
    ...
    const basePrice = 300; // Standard pricing
    setPrice(basePrice);
    ...
    <>
      ...
      <main className="bg-black text-white min-h-[80vh] flex flex-col justify-center items-center p-6">
        <div className="w-full max-w-md">
          <h1 className="text-3xl font-extrabold mb-2 text-center">Book Standard Ride</h1>
          <p className="text-gray-400 text-center mb-6">Reliable and comfortable rides for your daily transportation.</p>
          ...
          >
            <option value="">Select a Standard Vehicle</option>
