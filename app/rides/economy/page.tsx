...
export default function EconomyRidesPage() {
  const router = useRouter();
  const [pickup, setPickup] = useState("");
  const [dropoff, setDropoff] = useState("");
  const [vehicleClass] = useState("Economy");
  const [selectedVehicle, setSelectedVehicle] = useState("");
  const [vehicles, setVehicles] = useState<Vehicle[]>([]);
  const [price, setPrice] = useState<number | null>(null);
  const [message, setMessage] = useState("");

  // Fetch available Economy vehicles
  useEffect(() => {
  ...
  const handleBooking = async () => {
    ...
    const basePrice = 400; // Economy pricing
    setPrice(basePrice);
    ...
    <>
      ...
      <main className="bg-black text-white min-h-[80vh] flex flex-col justify-center items-center p-6">
        <div className="w-full max-w-md">
          <h1 className="text-3xl font-extrabold mb-2 text-center">Book Economy Ride</h1>
          <p className="text-gray-400 text-center mb-6">Budget-friendly rides for everyday travel needs.</p>
          ...
          >
            <option value="">Select an Economy Vehicle</option>
